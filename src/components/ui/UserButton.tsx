...
export const UserButton: React.FC<UserButtonProps> = ({ onToggleDrawer, showDrawer }) => {
  const { user, creditsRemaining, hasCredits } = useAuth();

  if (!user) return null;

  console.log('🔍 UserButton render:', { showDrawer, user: user.name });

  return (
    <>
      <button
        onClick={() => {
          console.log('🔘 UserButton clicked, toggling drawer');
          onToggleDrawer();
        }}
